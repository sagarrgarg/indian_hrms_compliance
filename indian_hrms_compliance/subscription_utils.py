...
def subscription_updated(app: str, plan: str):
	if app in ["indian_hrms_compliance", "erpnext"] and plan:
		update_erpnext_access()


def update_erpnext_access(user_input: dict | None):
	"""
	Called from hooks after setup wizard completion, ignored if user has no indian_hrms_compliance subscription
	enables erpnext workspaces and roles if user has subscribed to both indian_hrms_compliance and erpnext
	disables erpnext workspaces and roles if user has subscribed to indian_hrms_compliance but not erpnext
	"""
...
def set_app_logo():
	frappe.db.set_single_value("Navbar Settings", "app_logo", "/assets/indian_hrms_compliance/images/frappe-hr-logo.svg")


def get_erpnext_roles() -> set:
	erpnext_roles = get_roles_for_app("erpnext")
	hrms_roles = get_roles_for_app("indian_hrms_compliance")
