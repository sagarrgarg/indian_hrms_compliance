...
def delete_company_fixtures():
	...
	for country in countries:
		try:
			module_name = f"indian_hrms_compliance.regional.{frappe.scrub(country)}.setup.uninstall"
...
def run_regional_setup(country):
	try:
		module_name = f"indian_hrms_compliance.regional.{frappe.scrub(country)}.setup.setup"
		frappe.get_attr(module_name)()
	except ImportError:
		pass
	except Exception as e:
...
def make_salary_components(country):
	...
	if not frappe.db.exists("Salary Component", "Basic"):
		file_path = frappe.get_app_path("indian_hrms_compliance", "payroll", "data", file_name)
		docs.extend(json.loads(read_data_file(file_path)))

	file_path = frappe.get_app_path("indian_hrms_compliance", "regional", frappe.scrub(country), "data", file_name)
	docs.extend(json.loads(read_data_file(file_path)))
