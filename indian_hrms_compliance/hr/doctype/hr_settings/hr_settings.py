	...
	def validate_frequency_change(self):
		...
		try:
			weekly_job = frappe.get_doc(
				"Scheduled Job Type",
				{"method": "indian_hrms_compliance.controllers.employee_reminders.send_reminders_in_advance_weekly"},
			)

			monthly_job = frappe.get_doc(
				"Scheduled Job Type",
				{"method": "indian_hrms_compliance.controllers.employee_reminders.send_reminders_in_advance_monthly"},
			)
	...
	def show_freq_change_warning(self, from_date, to_date):
		...
		frappe.msgprint(
			...
			primary_action={
				...
				"client_action": "indian_hrms_compliance.proceed_save_with_reminders_frequency_change",
			},
