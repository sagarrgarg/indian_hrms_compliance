...
frappe.ui.form.on("Interview", {
	...
	submit_feedback: function (frm) {
		frappe.call({
			method: "indian_hrms_compliance.hr.doctype.interview.interview.get_expected_skill_set",
			args: {
	...
	show_feedback_dialog: async function (frm, data) {
		...
		let d = new frappe.ui.Dialog({
			...
			primary_action: function (values) {
				frappe
					.call({
						method: "indian_hrms_compliance.hr.doctype.interview.interview.create_interview_feedback",
						args: {
							data: values,
	...
	set_applicable_interviewers(frm) {
		frappe.call({
			method: "indian_hrms_compliance.hr.doctype.interview.interview.get_interviewers",
			args: {
				interview_round: frm.doc.interview_round || "",
			},
			callback: function (r) {
	...
	load_skills_average_rating(frm) {
		frappe
			.call({
				method: "indian_hrms_compliance.hr.doctype.interview.interview.get_skill_wise_average_rating",
	...
	load_feedback(frm) {
		frappe
			.call({
				method: "indian_hrms_compliance.hr.doctype.interview.interview.get_feedback",
				args: { interview: frm.doc.name },
			})
			.then((r) => {
