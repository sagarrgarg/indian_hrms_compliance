...
frappe.ui.form.on("Leave Application", {
	setup: function (frm) {
		frm.set_query("leave_approver", function () {
			return {
				query: "indian_hrms_compliance.hr.doctype.department_approver.department_approver.get_approvers",
				filters: {
	...
	onload: function (frm) {
		...
		if (frm.doc.docstatus == 0) {
			return frappe.call({
				method: "indian_hrms_compliance.hr.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function (r) {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.employee) {
			frappe.call({
				method: "indian_hrms_compliance.hr.doctype.leave_application.leave_application.get_leave_details",
				async: false,
				args: {
					employee: frm.doc.employee,
	...
	refresh: function (frm) {
		indian_hrms_compliance.leave_utils.add_view_ledger_button(frm);
		if (frm.is_new()) {
	...
	async set_employee(frm) {
		...
		const employee = await indian_hrms_compliance.get_current_employee(frm);
		if (employee) {
	...
	get_leave_balance: function (frm) {
		if (
			...
			return frappe.call({
				method: "indian_hrms_compliance.hr.doctype.leave_application.leave_application.get_leave_balance_on",
				args: {
					employee: frm.doc.employee,
	...
	calculate_total_days: function (frm) {
		if (frm.doc.from_date && frm.doc.to_date && frm.doc.employee && frm.doc.leave_type) {
			...
			return frappe.call({
				method: "indian_hrms_compliance.hr.doctype.leave_application.leave_application.get_number_of_leave_days",
				args: {
					employee: frm.doc.employee,
					leave_type: frm.doc.leave_type,
	...
	set_leave_approver: function (frm) {
		if (frm.doc.employee) {
			return frappe.call({
				method: "indian_hrms_compliance.hr.doctype.leave_application.leave_application.get_leave_approver",
				args: {
					employee: frm.doc.employee,
				},
				callback: function (r) {
	...
	set_form_buttons: async function (frm) {
		let self_approval_not_allowed = frm.doc.__onload
			...
			: 0;
		let current_employee = await indian_hrms_compliance.get_current_employee();
		if (
			frm.doc.docstatus === 0 &&
			!frm.is_dirty() &&
			!frappe.model.has_workflow(frm.doctype)
