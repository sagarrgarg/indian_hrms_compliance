...
def get_doc_condition(doctype):
	if doctype == "Compensatory Leave Request":
		...
		return "and (from_date between %(from_date)s and %(to_date)s \
...
def throw_overlap_error(doc, exists_for, overlap_doc, from_date, to_date):
	msg = (
		...
		+ (_(") for {0}").format(exists_for) if exists_for else ")")
	)
	frappe.throw(msg)
...
def generate_leave_encashment():
	...
	from indian_hrms_compliance.hr.doctype.leave_encashment.leave_encashment import create_leave_encashment
